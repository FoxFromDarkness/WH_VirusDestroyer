...
public class PlayerTriggerBehaviour : MonoBehaviour
{
    ...
    private void InfoTrigger_Enter(Collider2D collision)
    {
        if (collision.GetComponent<InfoTrigger>())
        {
            collision.GetComponent<InfoTrigger>().ShowInfoPanel();
        }
    }
